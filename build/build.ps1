...
# build the solution
$SolutionPath = "..\DocumentationAnalyzers.sln"
...
if (-not $SkipKeyCheck) {
	...
	foreach ($pair in $Keys.GetEnumerator()) {
		$assembly = Resolve-FullPath -Path "..\DocumentationAnalyzers\DocumentationAnalyzers.CodeFixes\bin\$BuildConfig\$($pair.Key)\DocumentationAnalyzers.dll"
		...
		$assembly = Resolve-FullPath -Path "..\DocumentationAnalyzers\DocumentationAnalyzers.CodeFixes\bin\$BuildConfig\$($pair.Key)\DocumentationAnalyzers.CodeFixes.dll"
		# Run the actual check in a separate process or the current process will keep the assembly file locked
		powershell -Command ".\check-key.ps1 -Assembly '$assembly' -ExpectedKey '$($pair.Value)' -Build '$($pair.Key)'"
		If (-not $?) {
			$host.ui.WriteErrorLine('Failed to verify strong name key for build, aborting!')
